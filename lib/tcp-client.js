  ...
  async connect () {
    ...
    this.connection.on('connect', async () => {
      ...
      const authentication = (data) => {
        //pulls login prompts from config
        if (data.toString('utf8').includes(`${this.opts.LoginPrompt}`)) return this.connection.write(`${this.opts.username}\r`)
        if (data.toString('utf8').includes(`${this.opts.PasswordPrompt}`)) return this.connection.write(`${this.opts.password}\r`)
        
        //these prompts are dependent on each system some systems have custom promts and menus
